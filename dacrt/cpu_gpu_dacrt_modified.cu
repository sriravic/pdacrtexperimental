...
extern "C" __global__ void modifiedSegmentedBruteForce(RayArray rays, TriangleArray triangles, int* buffered_ray_ids, int ray_buffer_occupied, int* buffered_tri_ids, int tri_buffer_occupied,
	int* ray_segment_sizes, int* tri_segment_sizes, int* ray_segment_start, int* tri_segment_start, int* segment_no, int* blockStart, float* maxts , int* hitids);
extern "C" void modifiedCompleteRender(ParallelPack& pack, TriangleArray& dev_triangles, RayArray& dev_rays, DacrtRunTimeParameters& rtparams, Counters& ctr);


// one global host only data
static thrust::host_vector<int> hbuffered_ray_idx_array;
static thrust::host_vector<int> hbuffered_tri_idx_array;
static thrust::host_vector<int> htri_segment_sizes;
static thrust::host_vector<int> hray_segment_sizes;
static thrust::host_vector<int> hsegment_ids;

// store the segment nos;
...
// changing the complete render method
extern "C"
void modifiedCompleteRender(ParallelPack& pack, TriangleArray& dev_triangles, RayArray& dev_rays, DacrtRunTimeParameters& rtparams, Counters& ctr) {
	// assuming all the host data has been spilled into the device memory.

	// copying only the blockNos and startBlockNos data
	static thrust::device_vector<int> dblockNos;
	static thrust::device_vector<int> dBlockStart;
	dblockNos.resize(blockCnt);
	dBlockStart.resize(blockCnt);

	Timer mem_cpy_timer1("Memcpy Timer");
	mem_cpy_timer1.start();
	thrust::copy(blockNos.begin(), blockNos.begin() + blockCnt, dblockNos.begin());
	thrust::copy(blockStart.begin(), blockStart.begin() + blockCnt, dBlockStart.begin());
	mem_cpy_timer1.stop();
	...
	hsegment_ids.clear(); blockNos.clear();
	//blockCnt = 0;			// reset this value
	//start = 0;

	// our original algorithm starts from here
	// copied code.!!
	...
	modifiedSegmentedBruteForce<<<num_blocks, num_threads_per_block>>>(dev_rays, dev_triangles, thrust::raw_pointer_cast(&pack.buffered_ray_idx[0]), pack.ray_buffer_occupied,
		...
		thrust::raw_pointer_cast(&dblockNos[0]), thrust::raw_pointer_cast(&dBlockStart[0]), thrust::raw_pointer_cast(&pack.buffered_ray_maxts[0]),
	...
	// reset our new temp static variables
	blockCnt = 0;			// reset this value
	bstart = 0;
	dBlockStart.clear();
	dblockNos.clear();
...
void cpuGpuDacrtSpatialFunctionModified(const AABB& space, 
	AabbArray& tri_aabbs,																							
	...
	Logger& logger
		...
		if(tpivot < rtparams.PARALLEL_TRI_THRESHOLD || rpivot < rtparams.PARALLEL_RAY_THRESHOLD) {
			...
			if((pack.ray_buffer_occupied + rpivot) < rtparams.BUFFER_SIZE && (pack.tri_buffer_occupied + tpivot) < rtparams.BUFFER_SIZE && pack.num_segments < rtparams.MAX_SEGMENTS) {
				...
				// block count is determined by the number of rays in each segment
				int numblocks = (rpivot / rtparams.NUM_RAYS_PER_BLOCK) + (rpivot % rtparams.NUM_RAYS_PER_BLOCK != 0);
				blockCnt += numblocks;
				// store num_segments for blockCnt times in blockNos starting from 
				// reset this value every time I am doing this.
				int tempstart = 0;
				for(int i = bstart; i < blockCnt; i++) {
					blockNos[i] = pack.num_segments;
					blockStart[i] = tempstart++;
				}
				...
				hbuffered_ray_idx_array.clear(); hbuffered_tri_idx_array.clear(); htri_segment_sizes.clear(); hray_segment_sizes.clear(); 
				hsegment_ids.clear(); blockNos.clear(); blockStart.clear();
				...
				// NEW CODE
				blockCnt += (rpivot / rtparams.NUM_RAYS_PER_BLOCK) + (rpivot % rtparams.NUM_RAYS_PER_BLOCK != 0);
				int tempstart = 0;
				for(int i = bstart; i < blockCnt; i++) {
					blockNos[i] = pack.num_segments;
					blockStart[i] = tempstart++;
			...
			float3 extents = space.bmax - space.bmin;
			{
				...
				cpuGpuDacrtSpatialFunctionModified(left, tri_aabbs, triangles, dev_triangles, num_triangles, tri_idx_array, newtpivot, rays, dev_rays, num_rays, ray_idx_array,
			...
			}
			{
				...
				cpuGpuDacrtSpatialFunctionModified(right, tri_aabbs, triangles, dev_triangles, num_triangles, tri_idx_array, newtpivot, rays, dev_rays, num_rays, ray_idx_array, newrpivot,
...
void cpuGpuDacrtSpatialPartitioningModified(const AABB& sceneBox, 
	...
	Logger& logger
		...
		// very crude but we have to check this out.!
		...
		cpuGpuDacrtSpatialFunctionModified(sceneBox, tri_aabbs, triangles, dev_triangles, num_triangles, tri_idx_array, tpivot, rays, dev_rays, num_rays, ray_idx_array, rpivot,
			pack, maxts, hitids, rtparams, ctr, logger);
		...
		if(pack.num_segments > 0) 
			modifiedCompleteRender(pack, dev_triangles, dev_rays, rtparams, ctr);
